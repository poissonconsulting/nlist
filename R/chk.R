...
chk_natomic <- function(x, err = TRUE, x_name = NULL) {
  ...
  err("`", x_name, "`", 
      " must be a numeric (integer or double)",
...
chk_nlist <- function(x, err = TRUE, x_name = NULL) {
  if(is.list(x) && !is.null(names(x)) && !anyDuplicated(names(x)) && 
     all(vapply(x, chk_natomic, TRUE, err = FALSE)))
    return(TRUE)
  if(!err) return(FALSE)
  if(is.null(x_name)) x_name <- deparse(substitute(x))
  err("`", x_name, "`", 
      " must be a uniquely named list of numeric (integer or double)",
      " atomic (vector, matrix or array) objects.")
}
...
chk_nlists <- function(x, err = TRUE, x_name = NULL) {
  ...
  if(!all(vapply(x, chk_nlist, TRUE, err = FALSE))) {
    ...
    if(is.null(x_name)) x_name <- deparse(substitute(x))
    err("`", x_name, "`", 
        " must be a list of nlist objects.")    
  }
