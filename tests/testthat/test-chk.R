...
test_that("chk_natomic", {
  ...
  expect_error(chk_natomic(NULL),
    "^`NULL` must be a numeric [(]integer or double[)] atomic [(]vector, matrix or array[)] object[.]", class = "chk_error")
...
test_that("chk_nlist", {
  ...
  expect_error(chk_nlist(list(x = TRUE)),
    "`list[(]x = TRUE[)]` must inherit from S3 class 'nlist'[.]", class = "chk_error")
})
...
test_that("chk_nlists", {
  ...
  expect_error(nlists(nlist(x = 1), nlist(y = 2)),
    "^nlist elements of `x` must have matching names[.]", class = "chk_error")
  ...
  expect_error(nlists(nlist(x = 1L), nlist(x = 1:2)),
    "^nlist elements of `x` must have matching dimensions[.]", class = "chk_error")
})
