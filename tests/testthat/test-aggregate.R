...
test_that("aggregate.nlists", {
  expect_error(aggregate(nlists(nlist(x = matrix(1:9, 3)), 
                                    nlist(x = matrix(2:10, 3))), fun = identity),
                   "^`fun` must return a scalar[.]$")
})

test_that("aggregate.nlists by_chain = TRUE", {
  expect_identical(aggregate(nlists(), by_chain = TRUE), 
                   structure(list(), class = "nlists"))
  expect_identical(aggregate(nlists(nlist()), by_chain = TRUE), 
                   structure(list(structure(list(), .Names = character(0), class = "nlist")), class = "nlists"))
  expect_identical(aggregate(nlists(nlist(x = 1), nlist(x = 2)), by_chain = TRUE), 
                   structure(list(structure(list(x = 1.5), class = "nlist")), class = "nlists"))
  expect_identical(aggregate(nlists(nlist(x = matrix(1:9, 3)), 
                                    nlist(x = matrix(2:10, 3))), by_chain = TRUE), 
                   structure(list(structure(list(x = structure(c(1.5, 2.5, 
3.5, 4.5, 5.5, 6.5, 7.5, 8.5, 9.5), .Dim = c(3L, 3L))), class = "nlist")), class = "nlists"))
